...
describe('parseDshArgs', () => {
  it('routes profile boots and the surface aliases, handing the rest to the app', () => {
    ...
    expect(parse(['gstar'])).toEqual({ mode: 'profile', profile: 'gstar', patches: [], args: [] })
    expect(parse(['gstar', '--patch', 'gstar.yml']))
      .toEqual({ mode: 'profile', profile: 'gstar', patches: ['gstar.yml'], args: [] })
  })
  ...
  it('ends the launcher flags at the first token it does not own', () => {
    ...
    expect(parse(['gstar', '--host', '127.0.0.1', '--port', '8081', '--no-open']))
      .toEqual({ mode: 'profile', profile: 'gstar', patches: [], args: ['--host', '127.0.0.1', '--port', '8081', '--no-open'] })
  ...
  it('routes profile and web config dumps', () => {
    ...
    expect(parse(['web', '--dump-default-config']))
      .toEqual({ mode: 'dump-config', profile: 'web', defaultOnly: true, patches: [] })
    expect(parse(['gstar', '--dump-config']))
      .toEqual({ mode: 'dump-config', profile: 'gstar', defaultOnly: false, patches: [] })
    expect(parse(['gstar', '--dump-default-config']))
      .toEqual({ mode: 'dump-config', profile: 'gstar', defaultOnly: true, patches: [] })
  })
  ...
  it('rejects missing profile, removed flags, and contradictory inputs', () => {
    ...
    expect(exitCode(['gstar', '--dump-config', '--dump-default-config'])).toBe(1)
    expect(exitCode(['gstar', '--dump-default-config', '--patch', 'g.yml'])).toBe(1)
    expect(exitCode(['gstar', '--patch='])).toBe(1)
