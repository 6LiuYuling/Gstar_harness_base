...
| `@deepseek-ai/dsh-gstar-tool-spatial-query` | `gstar_station_data` | `ctx.tools`, `ctx.gstarSites`, `ctx.gstarSpatial`, `a calling station Agent` | `tool/call`, `tool/result` | - | The model never supplies station identity; execution resolves the exact station from the calling Session cwd. |
...
Read the current GSTAR station, its AOIs, entity fields, and acquisition provenance. Omit aoi_id for an overview; provide it for entity-level data.
...
Source: [`packages/gstar/tool-spatial-query/src/index.ts`](../packages/gstar/tool-spatial-query/src/index.ts)

The model never supplies station identity; execution resolves the exact station from the calling Session cwd.
