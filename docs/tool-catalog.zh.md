...
| `@deepseek-ai/dsh-gstar-tool-spatial-query` | `gstar_station_data` | `ctx.tools`、`ctx.gstarSites`、`ctx.gstarSpatial`、`a calling station Agent` | `tool/call`、`tool/result` | - | 模型不会提供局点身份；执行时从调用 Session 的 cwd 解析精确局点。 |
...
<a id="deepseek-aidsh-gstar-tool-spatial-query"></a>

## `@deepseek-ai/dsh-gstar-tool-spatial-query`

### `gstar_station_data`

读取当前 GSTAR 局点、AOI、实体字段和采集溯源。省略 aoi_id 时返回概览；提供 aoi_id 时返回实体级数据。
...
```json
{
  ...
  "properties": {
    "aoi_id": {
      "type": "string",
      "description": "AOI id from a station overview. Omit to list AOIs."
    },
    "entity_limit": {
      "type": "integer",
      "description": "Maximum entities returned for one AOI, from 1 to 200."
    }
  }
}
```

来源：[`packages/gstar/tool-spatial-query/src/index.ts`](../packages/gstar/tool-spatial-query/src/index.ts)

模型不会提供局点身份；执行时从调用 Session 的 cwd 解析精确局点。
