...
GSTAR 将选定的 DSH Workspace 分类为局点。每个局点拥有独立空间投影，其中包括 WGS84 位置、可选行政区边界、分类 AOI 多边形、实体属性和不可变的数据来源记录。删除局点只清理 GSTAR 自有记录，底层 Workspace 与 Session 保持不变。

数据获取由单独的动态组合层负责。数据源插件向 `ctx.gstarDataSources` 注册元数据；直接数据源还会注册同步操作。存储提供方只持久化逐局点的启用覆盖，因此加载或卸载数据源插件会立即改变实时目录，无需重写局点记录。数据源被禁用、仅供参考或已卸载时，同步请求会被拒绝。

默认 Profile 将 OpenStreetMap 组合为已启用的 AOI 数据源，将 AKShare 组合为可选的上市公司增强数据源。政务公开平台因缺少统一的机器可读获取约定，以默认禁用的参考源加载，用于核验。每次发布的 AOI 或增强结果都保留底层来源标识和获取时间。
