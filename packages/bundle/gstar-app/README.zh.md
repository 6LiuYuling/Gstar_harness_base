...
覆盖层挂载 Workspace 支撑的局点 Provider、`storage-domain` 空间 Provider、Cesium 资产 Host 路由以及 GSTAR 专用 Client Remote 组合。`ctx.gstarSites` 只发布具有持久化 GSTAR 成员关系的 Workspace；`ctx.gstarSpatial` 持有它们的位置、AOI、实体与溯源。普通 `dsh web` Workspace 保持在两种 GSTAR 投影之外。
...
- 底图影像目前使用公共 OpenStreetMap 瓦片，需要网络访问。
