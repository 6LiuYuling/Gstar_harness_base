...
覆盖层挂载 Workspace 支撑的局点 Provider、`storage-domain` 空间 Provider、按有界 Overpass 响应配置的固定来源 Host HTTP Provider、Cesium 资产 Host 路由以及 GSTAR 专用 Client Remote 组合。`ctx.gstarSites` 只发布具有持久化 GSTAR 成员关系的 Workspace；`ctx.gstarSpatial` 根据局点名称自动解析并保存位置，从 OpenStreetMap 采集分类 AOI，编目权威公开参考源，并持有 AOI 实体与溯源。普通 `dsh web` Workspace 保持在两种 GSTAR 投影之外。
...
- 官方来源专用采集、数据处理、带版本流水线执行与变更型 Agent 工具仍属于后续 GSTAR 能力；随附的 `gstar_station_data` 工具只读。
