...
`locate()` 通过注入的 DSH `ctx.web` 能力依次尝试 Nominatim 与 Photon，优先去掉中文「局点/站点」后缀并在未命中时回退到完整名称，校验返回的 WGS84 坐标，并通过同一空间写入路径持久化。传输、HTTP 与异常载荷失败会继续尝试下一个 Host 提供方，并保留原因链用于 Remote 诊断。浏览器不会直接发起地理编码请求。
