...
`locate()` 通过注入的 DSH `ctx.web` 能力访问固定的 Nominatim 端点，优先去掉中文「局点/站点」后缀并在未命中时回退到完整名称，校验返回的 WGS84 坐标，并通过同一空间写入路径持久化。浏览器不会直接发起地理编码请求。
...
- 公共 Nominatim 的可用性与使用政策仍是外部部署依赖；生产部署可在同一 Service Definition 后替换 Provider。
