...
The bridge reads `profileDatabasePath` first when a persistent AKShare/CNInfo CSV is configured. It accepts the normalized columns produced by a deployment's regional-company collector, matches company names and aliases, and filters registered or office addresses without a remote request. Display names such as `Guangzhou · Nansha` and canonical administrative names are normalized into city and district address tokens; a province token is optional when narrower levels exist. All same-name CSV rows are considered before the address-qualified row is selected. Local results report candidate-AOI, name-match, and address-match counts. Otherwise the bridge tries AKShare's exchange, Eastmoney, Tencent, and Sina complete A-share lists in order, retries transport and response-decoding failures, and requests each matched CNInfo profile under the same retry policy. If every list source or a required profile request is unavailable, synchronization reports that no update occurred and preserves the complete existing AOI publication. The source does not create geometry, merge a group parent with a listed subsidiary, or copy a company into a neighboring district.
