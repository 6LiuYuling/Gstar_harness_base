...
AKShare enriches existing enterprise or financial AOIs rather than creating geometry. Its bounded Python bridge first reads a configured persistent AKShare/CNInfo CSV and resolves company names, aliases, and station addresses without network access. Display and canonical station titles normalize to suffix-independent city and district tokens, province tokens become optional when narrower levels exist, and every same-name row remains eligible until address qualification selects one. Local results expose candidate-AOI, name-match, and address-match counts. Without that file, the bridge retries response and transport failures across the exchange, Eastmoney, Tencent, and Sina complete A-share lists, then retrieves the corresponding CNInfo company profile under the same retry bound. If all complete lists or a required profile request are unavailable, synchronization preserves the existing AOI publication and returns an explicit no-update result. One complete spatial patch adds accepted `listed_company` entities and source provenance only after every required bridge result succeeds. TLS verification is strict by default: deployments may supply a Requests CA bundle, while an explicit mutually exclusive insecure switch affects only the isolated bridge child. The connector does not merge a group parent with a listed subsidiary or admit an address from a neighboring district.
...
Service and Loader-composition tests cover Remote delegation, live source registration, default and overridden enablement, station rejection, disabled and reference-only execution, OSM delegation, deletion rollback, and disposal. Source tests cover reference URL validation, OSM summaries, persistent database input, display and canonical station titles, same-name address selection, local funnel diagnostics, four-source AKShare fallback, explicit no-update results, request-retry bounds, strict and custom-CA TLS configuration, explicit insecure isolation, concise transport and certificate diagnostics, output validation, publication atomicity, entity provenance, and missing station or spatial projections. Client tests cover three Remote mounts, station-aware stale-load protection, toggle and synchronization errors, OSM policy routing, and the source-manager controls.
