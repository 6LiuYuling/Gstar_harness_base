...
`dsh-gstar-client-remotes` 只为 `gstar` Profile 选择生成的局点 contribution，并将其挂载到 DSH 标准 Client Remote carrier。`ui-gstar` 向纯根组件注入 `createSite` action；该 action 经过 `ctx.remote.gstarSites.create`，实时列表读取继续使用标准 Workspace 投影。通用 `dsh web` 组合保持不变。
...
GSTAR Client 组合测试固定生成 contribution 的挂载与销毁。UI apply 测试固定精确 Remote 依赖及成功/错误信封处理；组件测试通过行内表单驱动注入的局点创建 action。

包级 Model Experience 审计把 GSTAR Bundle、UI、Service Definition、Workspace Provider 和 Client Remote 组合归类为模型中性 contribution。

## Consequences

第一阶段 `gstar` 组合无需复制 Web 基础设施或修改通用 Workspace 约定，即可启动为基于真实 Workspace 的局点界面。产品导航可以先于各数据能力出现，同时每个尚不可用的领域都保持明确。局点创建现已跨越 DSH Host/Remote/Client 边界，并通过 Workspace 持久化。暂时代价是数据源、门禁和流水线入口仍显示服务占位，而且在 directory-picker contribution 挂载前，创建表单需要填写 Host 路径。
